...
字符串内可以使用 `${expression}` 语法插入变量值或表达式结果。
...
支持任意项目级别的有效值表达式，包括变量引用、函数参数、环境变量等。

| 示例表达式                 | 说明                |
|-----------------------|-------------------|
| `${name}`             | 引用当前作用域中的变量       |
| `${input.name}`       | 引用 function 的输入参数 |
| `${output.name}`      | 引用 function 的输出参数 |
| `${env.VAR_NAME}`     | 引用环境变量            |
| `${block.label.attr}` | 引用块属性             |
...
| defaultPermission  | 字符串 | 否  | 默认权限，可选 `read-only`、`searchable`                     |
| private            | 对象  | 否  | 认证配置                                                 |
| private.authType   | 字符串 | 是  | 认证类型：JWT、Basic、OAuth2、RFC5054、publicKey              |
| public             | 对象  | 否  | 设置重试，failback 等策略                                    |
| public.retryPolicy | 字符串 | 否  | 定义重试策略，可选以下值：`always`、`none`、`exponential-backoff` |
...
settings 块用于定义全局配置项，支持任意键值对。

若没有声明将使用默认值。
如果一个 Value 已经声明但没有使用，它将会被丢弃。
